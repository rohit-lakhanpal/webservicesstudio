    ...
    internal class ProxyProperties
    {
        ...
        private void InitAdditionalProperties(HttpWebClientProtocol proxy)
        {
            if (proxyTypeHandlers == null)
            {
                ...
                var proxyProperties = Configuration.MasterConfig.ProxyProperties;
                if ((proxyProperties != null) && (proxyProperties.Length > 0))
                {
                    foreach (var handler in proxyProperties)
                    {
                        var typeName = handler.TypeName;
                        var str2 = handler.Handler;
                        if (((typeName != null) && (typeName.Length != 0)) && ((str2 != null) && (str2.Length != 0)))
                        {
                            var key = Type.GetType(typeName);
                            if (key == null)
                            {
            ...
            for (var type3 = proxy.GetType(); type3 != typeof(object); type3 = type3.BaseType)
            {
        ...
        public void UpdateProxy(HttpWebClientProtocol proxy)
        {
            ...
            {
                var uri = new Uri(this.HttpProxy.Url);
                if (proxy.Proxy == null)
                {
