namespace WebServiceStudio
{
    ...
    internal class TreeNodeProperty
    {
        ...
        public void AddToTypeList(object o)
        {
            ...
            var destinationArray = new System.Type[this.types.Length + 1];
        ...
        {
            var type = list[0] as System.Type;
            foreach (var type2 in includedTypes)
            {
        ...
        {
            var elementType = (val == null) ? possibleTypes[0] : val.GetType();
        ...
        {
            var typeArray = includedTypesLookup[webService] as System.Type[];
            if (typeArray == null)
            {
                var list = new ArrayList();
                var customAttributes = webService.GetCustomAttributes(typeof(SoapIncludeAttribute), true) as SoapIncludeAttribute[];
                foreach (var attribute in customAttributes)
                {
                    list.Add(attribute.Type);
                }
                var attributeArray2 = webService.GetCustomAttributes(typeof(XmlIncludeAttribute), true) as XmlIncludeAttribute[];
                foreach (var attribute2 in attributeArray2)
                {
