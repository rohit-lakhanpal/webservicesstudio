namespace WebServiceStudio
{
    ...
    internal class ListStandardValues : TypeConverter
    {
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            if (value is string)
            {
                var possibleValues = this.GetPossibleValues(context);
                if (possibleValues != null)
                {
                    foreach (var obj2 in possibleValues)
                    {
        ...
        {
            var method = context.Instance.GetType().GetMethod("Get" + context.PropertyDescriptor.Name + "List");
            if (method == null)
            {
                return null;
            }
