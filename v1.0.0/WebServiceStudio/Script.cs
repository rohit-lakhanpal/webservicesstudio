namespace WebServiceStudio
{
    ...
    internal class Script
    {
        ...
        private CodeExpression BuildArray(CodeStatementCollection statements, string name, object value)
        {
            ...
            var uniqueVariableName = GetUniqueVariableName(name, statements);
            ...
            for (var i = 0; i < array.Length; i++)
            {
                ...
                var right = this.BuildObject(statements, str2 + i.ToString(), array.GetValue(i));
        ...
        private CodeExpression BuildClass(CodeStatementCollection statements, string name, object value)
        {
            ...
            statements.Add(statement);
            var targetObject = new CodeVariableReferenceExpression(uniqueVariableName);
            foreach (var info in type.GetMembers(BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance))
            {
                object obj2 = null;
                ...
                else if (info is PropertyInfo)
                {
                    ...
                    var getMethod = info3.GetGetMethod();
                ...
                if (left != null)
                {
                    var right = this.BuildObject(statements, info.Name, obj2);
                    statements.Add(new CodeAssignStatement(left, right));
                }
            Label_014B:;
            }
        ...
        {
            var expression = new CodeMethodInvokeExpression(this.dumpMethodRef, new CodeExpression[0]);
        ...
        private void BuildMethod(CodeTypeMemberCollection members, MethodInfo method, object[] parameters)
        {
            ...
            members.Add(method2);
            var expression = new CodeMethodInvokeExpression(this.BuildProxy(method2.Statements, method), method.Name, new CodeExpression[0]);
            ...
            {
                var uniqueVariableName = GetUniqueVariableName(method.Name + "Result", method2.Statements);
            ...
            var infoArray = method.GetParameters();
        ...
        private CodeExpression BuildObject(CodeStatementCollection statements, string name, object value)
        {
            ...
            if (c.IsEnum)
            {
                var strArray = value.ToString().Split(new char[] { ',' });
            ...
            if (c == typeof(DateTime))
            {
                ...
                var str = ((DateTime) value).ToString("yyyy-MM-ddTHH:mm:ss.fffffffzzzzzz", DateTimeFormatInfo.InvariantInfo);
        ...
        private void BuildParameters(CodeStatementCollection statements, MethodInfo method, object[] paramValues, CodeExpressionCollection parameters)
        {
            ...
            {
                var info = infoArray[i];
                var parameterType = infoArray[i].ParameterType;
        ...
        {
            var type = this.proxy.GetType();
            var name = CodeIdentifier.MakeCamel(type.Name);
            ...
            for (var i = 0; i < soapHeaders.Length; i++)
            {
                var info = soapHeaders[i];
                if (typeof(SoapHeader).IsAssignableFrom(info.FieldType))
                {
                    ...
                    var right = this.BuildObject(statements, info.Name, info.GetValue(this.proxy));
                    statements.Add(new CodeAssignStatement(left, right));
                }
            }
            return targetObject;
        ...
        private CodeExpression BuildXmlNode(CodeStatementCollection statements, string name, object value)
        {
            ...
            if (type == typeof(XmlElement))
            {
                ...
                var str = GetUniqueVariableName(name + "Doc", statements);
            ...
            var uniqueVariableName = GetUniqueVariableName(name + "Doc", statements);
            ...
            var str3 = GetUniqueVariableName(name + "Attr", statements);
        ...
        {
            var customAttributes = type.GetCustomAttributes(typeof(CLSCompliantAttribute), true) as CLSCompliantAttribute[];
