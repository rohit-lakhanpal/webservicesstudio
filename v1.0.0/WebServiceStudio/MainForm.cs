    ...
    public class MainForm : Form
    {
        ...
        private void buttonGet_Click(object sender, EventArgs e)
        {
            if (this.buttonGet.Text == "Get")
            {
                ...
                var text = this.textEndPointUri.Text;
                this.wsdl.Reset();
        ...
        private void CopyToClipboard(TreeNodeProperty tnp)
        {
            ...
            var o = tnp.ReadChildren();
            if (o != null)
            {
                var writer = new StringWriter();
                var extraTypes = new System.Type[] { o.GetType() };
                var type = (o is DataSet) ? typeof(DataSet) : typeof(object);
        ...
        {
            var proxyAssembly = this.wsdl.ProxyAssembly;
            if (proxyAssembly != null)
            {
                ...
                foreach (var type in proxyAssembly.GetTypes())
                {
                    ...
                    {
                        var node = this.treeMethods.Nodes.Add(type.Name);
        ...
        private void FillWsdlTab()
        {
            if ((this.wsdl.Wsdls != null) && (this.wsdl.Wsdls.Count != 0))
            {
                ...
                this.treeWsdl.Nodes.Clear();
                var node = this.treeWsdl.Nodes.Add("WSDLs");
                var writer = new XmlTreeWriter();
                for (var i = 0; i < this.wsdl.Wsdls.Count; i++)
                {
                    num3 = i + 1;
                    var root = node.Nodes.Add("WSDL#" + num3.ToString());
                    root.Tag = this.wsdl.Wsdls[i];
                    writer.FillTree(this.wsdl.Wsdls[i], root);
                }
                var node3 = this.treeWsdl.Nodes.Add("Schemas");
                for (var j = 0; j < this.wsdl.Xsds.Count; j++)
                {
                    num3 = j + 1;
                    var node4 = node3.Nodes.Add("Schema#" + num3.ToString());
        ...
        {
            var script = new Script(this.wsdl.ProxyNamespace, "MainClass");
        ...
        private MethodProperty GetCurrentMethodProperty()
        {
            ...
            var node = this.treeInput.Nodes[0];
        ...
        {
            var currentMethodProperty = this.GetCurrentMethodProperty();
            if (currentMethodProperty != null)
            {
                var proxy = currentMethodProperty.GetProxyProperty().GetProxy();
                var properties = new RequestProperties(proxy);
                try
                {
                    var method = currentMethodProperty.GetMethod();
                    var declaringType = method.DeclaringType;
                    WSSWebRequest.RequestTrace = properties;
                    var parameters = currentMethodProperty.ReadChildren() as object[];
                    var result = method.Invoke(proxy, BindingFlags.Public, null, parameters, null);
                    this.treeOutput.Nodes.Clear();
                    var property2 = new MethodProperty(currentMethodProperty.GetProxyProperty(), method, result, parameters);
        ...
        {
            var version = typeof(string).Assembly.GetName().Version;
        ...
        private void menuItemOpen_Click(object sender, EventArgs e)
        {
            ...
            var fileName = this.openWsdlDialog.FileName;
            var cursor = this.Cursor;
            this.Cursor = Cursors.WaitCursor;
            try
            {
        ...
        private void menuItemSaveAll_Click(object sender, EventArgs e)
        {
            ...
            {
                var length = this.saveAllDialog.FileName.LastIndexOf('.');
                var str = (length >= 0) ? this.saveAllDialog.FileName.Substring(0, length) : this.saveAllDialog.FileName;
        ...
        {
            var width = (this.panelRightInvoke.Width - 0x18) / 2;
            var x = 8;
            var num3 = (8 + width) + 8;
            var height = (((this.panelRightInvoke.Height - 0x10) - 20) - 40) / 2;
            var y = 8;
            var num6 = (0x1c + height) + 20;
        ...
        {
            var width = this.panelRightRaw.Width - 0x10;
            var x = 8;
            var height = (((this.panelRightRaw.Height - 0x10) - 20) - 40) / 2;
            var y = 8;
            var num5 = (0x1c + height) + 20;
        ...
        {
            var selectedObject = this.propInput.SelectedObject as TreeNodeProperty;
            if ((selectedObject != null) && ((e.ChangedItem.Label == "Type") && (e.OldValue != selectedObject.Type)))
            {
                var property2 = TreeNodeProperty.CreateTreeNodeProperty(selectedObject);
        ...
        private void SendWebRequest()
        {
            ...
            var selectedObject = this.propRequest.SelectedObject as RequestProperties;
            ...
            var proxy = this.GetCurrentMethodProperty().GetProxyProperty().GetProxy();
        ...
        {
            var handler = new ResolveEventHandler(this.OnAssemblyResolve);
        ...
        private void tabMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var node = this.treeWsdl.Nodes[3];
        ...
        private void textEndPointUri_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (!char.IsControl(e.KeyChar))
            {
                ...
                {
                    for (var i = 0; i < this.textEndPointUri.Items.Count; i++)
                    {
        ...
        {
            var tag = this.treeInput.SelectedNode.Tag as TreeNodeProperty;
            if (tag is MethodProperty)
            {
                throw new Exception("Paste not valid on method");
            }
            var typeList = tag.GetTypeList();
            var type = typeof(DataSet).IsAssignableFrom(typeList[0]) ? typeof(DataSet) : typeof(object);
            ...
            var val = serializer.Deserialize(textReader);
            if ((val == null) || !typeList[0].IsAssignableFrom(val.GetType()))
            {
                throw new Exception("Invalid Type pasted");
            }
            var property2 = TreeNodeProperty.CreateTreeNodeProperty(tag, val);
        ...
        private void treeMethods_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (e.Node.Tag is MethodInfo)
            {
                ...
                this.treeInput.Nodes.Clear();
                var property = new MethodProperty(this.GetProxyPropertyFromNode(e.Node), tag);
                property.RecreateSubtree(null);
